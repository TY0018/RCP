...
def compute_features(
    ...
    try:
        ...
        sig = sound.select_bandwidth(sig,
                                    params["SAMPLE_RATE"],
        ...
        # 3. compute the spectrogram
        #----------------------------------------------
        Sxx, tn, fn, ext = sound.spectrogram(sig,
                                             params["SAMPLE_RATE"],
                                             nperseg=params["NFFT"],
        ...
        #----------------------------------------------
        Sxx_clean_dB, _, _ = sound.remove_background(Sxx_dB)
        ...
        if (len(np.where(sound.avg_amplitude_spectro(Sxx_clean_dB) != 0)[0])) <= 2:
            min_y = 0
            max_y = Sxx_clean_dB.shape[1]
        else:
            min_y = np.where(sound.avg_amplitude_spectro(Sxx_clean_dB) != 0)[0][2]  # 2 to avoid to take indice 0 or 1
            max_y = np.where(sound.avg_amplitude_spectro(Sxx_clean_dB) != 0)[0][-1]
        ...
        #----------------------------------------------
        df_shape, params_shape = features.shape_features(Sxx_clean_dB, 
        ...
        mean_spectrum = maad.util.running_mean(sound.avg_power_spectro(Sxx_clean), N=35)
...
def multicpu_compute_features(
    ...
    if len(df_rois[~mask]) > 0 :
        ...
        if (file_exist == False) or ((file_exist == True) and (overwrite == True)) : 
            ...
            with tqdm(total=len(df_rois[~mask])) as pbar:
                with futures.ProcessPoolExecutor(max_workers=nb_cpu-1) as pool:
                    for df_features_temp in pool.map(
                        ...
                        pbar.update(1)
                        df_features = pd.concat([df_features, df_features_temp],ignore_index=True)
            ...
            drop_col = df_features.columns.intersection(df_rois.columns).tolist()
            ...
            for categories in df_features["categories"].unique():
                new_rows = df_features[df_features["categories"] == categories].sort_index()

                # Concatenate it to the existing dataframe
                df_features_sorted = pd.concat([df_features_sorted, new_rows], ignore_index=False)
