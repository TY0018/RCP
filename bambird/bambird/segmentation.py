...
def _save_rois(
    ...
    for index, row in df_rois.iterrows():
        ...
        if fcut_max > fs / 2:
            fcut_max = row["max_f"]
        # cut in frequency
        chunk_rois = sound.select_bandwidth(
            chunk_rois,
            fs,
...
def safe_load_audio(path, sr, duration):
    try:
        import librosa
        return librosa.load(path, sr=sr, duration=duration)
    except Exception:
        # Fallback: convert on-the-fly to wav
...
def single_file_extract_rois(
    ...
    try :
        ...
        if len(parts) >= 2:
            categories = (parts[0][:3] + parts[1][:3]).lower()
        else:
            categories = parts[0][:3].lower() if parts else "unk"
            print(f"Categories from file: {categories}")
    except :
        _, species = os.path.split(path)
        categories = species
        print(f"Couldn't get short categories: {categories}")
    
    try:
        # 1. load audio
        sig, sr = safe_load_audio(
            ...
            for chunk in sig_splits:
                ...
                if len(current_df_rois) > 0 :
                    ...
                    if len(df_rois) > 0:
                        ...
                        df_rois = pd.concat([df_rois, current_df_rois], ignore_index=True)
                    else:
    ...
    if (dir_exist == False) or ((dir_exist == True) and (overwrite == True)) : 
        ...
        if len(df_data[~mask])>0 :
            ...
            with tqdm(total=len(df_data[~mask])) as pbar:
                with futures.ProcessPoolExecutor(max_workers=nb_cpu-1) as pool:
                    for df_rois_temp in pool.map(
                        ...
                        pbar.update(1)
                        # df_rois = df_rois._append(df_rois_temp)
                        df_rois = pd.concat([df_rois, df_rois_temp], ignore_index=True)
            ...
            for categories in df_rois["categories"].unique():
                ...
                # )
                df_rois_sorted = pd.concat([df_rois_sorted, df_rois[df_rois["categories"] == categories].sort_index()],ignore_index=True)
