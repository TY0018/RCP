...
PARAMS_EXTRACT = {'SAMPLE_RATE': 32000,
                 'LOW_FREQ': 250,
                 'HIGH_FREQ': 12000,
                 'BUTTER_ORDER': 1,
                 'AUDIO_DURATION': 30,
    ...
    if rois is None: 
        ...
        if im_rois is not None : 
            ...
            for _, row in rois.iterrows() : 
                ...
                im_blobs = rois.rois_to_imblobs(np.zeros(Sxx.shape), row)     
...
def _select_rois(im_bin, 
    ...
    >>> im_rois, df_rois = rois.select_rois(im_bin, display=True)
    ...
    >>> im_bin2 = rois.create_mask(Sxx_noNoise_dB_blurred, bin_std=6, bin_per=0.5, mode='relative') 
    >>> im_rois2, df_rois2 = rois.select_rois(im_bin2, display=True)
